...
module.exports = class Lyricist {
  ...
  */
  ...
  async song(id, { fetchLyrics = false, textFormat = 'dom' } = {}) {
    ...
    const path = `songs/${id}?text_format=${textFormat}&access_token=${this.accessToken}`;
  ...
  */
  ...
  async album(id, { fetchTracklist = false, textFormat = 'dom' } = {}) {
    ...
    const path = `albums/${id}?text_format=${textFormat}&access_token=${this.accessToken}`;
  ...
  async artist(id, { textFormat = 'dom' } = {}) {
    ...
    const path = `artists/${id}?text_format=${textFormat}&access_token=${this.accessToken}`;
  ...
  */
  ...
  async songsByArtist(id, { page = 1, perPage = 20, sort = 'title' } = {}) {
    ...
    const path = `artists/${id}/songs?per_page=${perPage}&page=${page}&sort=${sort}&access_token=${this.accessToken}`;
  ...
  async search(query) {
    ...
    const path = `search?q=${query}&access_token=${this.accessToken}`;
  ...
  async _scrapeLyrics(url) {
    const response = await axios.get(url + `?access_token=${this.accessToken}`);
